export const Logo =
  "https://help.nflxext.com/helpcenter/OneTrust/oneTrust_production_2025-07-01/consent/87b6a5c0-0104-4e96-a291-092c11350111/01938dc4-59b3-7bbc-b635-c4131030e85f/logos/dd6b162f-1a32-456a-9cfe-897231c7763c/4345ea78-053c-46d2-b11e-09adaef973dc/Netflix_Logo_PMS.png";

export const API_OPTIONS = {
  method: "GET",
  headers: {
    accept: "application/json",
    Authorization:
      "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiZWIwYjgwZGViMTk2NjlhNTEyNDcwYTAyMzc3NDQ5NSIsIm5iZiI6MTc1NDA3MDYyOS40NjEsInN1YiI6IjY4OGNmZTY1ODMxYmU1YTkyMGI3ZTg0NyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.0dGKYU7sJYylU1AeuX11asB_avBTakmTEj9SqUZSt4A",
  },
};


export const IMG_CDN_URL ="https://image.tmdb.org/t/p/w400"